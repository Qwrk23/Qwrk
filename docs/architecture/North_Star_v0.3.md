...
**v0.3 — 2026-01-19:** Added `instruction_pack` as a Kernel v1 artifact type to support external instruction extensions for GPT front-ends. Defined extension table, scope constraints, and mutability rules.

**v0.2 — 2026-01-18:** Added Branch and Leaf as first-class artifact types to support project execution anatomy (Tree/Sapling → Branch → Leaf). Clarified parent/child governance and explicitly excluded Flowers from project execution trees.

**v0.1 — 2025-12-30:** Initial North Star document. Established Kernel v1 scope (project, snapshot, restart, journal), artifact spine model, and governance principles.
...
- Kernel v1 focuses on **Project + Snapshot** with supporting types **Restart**, **Journal**, and **Instruction Pack**
...
- Type tables: `Qxb_Artifact_Project`, `Qxb_Artifact_Snapshot`, `Qxb_Artifact_Restart`, `Qxb_Artifact_Journal`, `Qxb_Artifact_Instruction_Pack`, `Qxb_Artifact_Video` (added post-v1)
...
| `artifact_type` | text | Allow-listed type identifier (Kernel v1: project, snapshot, restart, journal, instruction_pack) |
...
Additional first-class artifact types may exist in the system and database but are not required for Kernel v1 MVP execution until explicitly scheduled. These include structural containers (e.g., forest, thicket) and execution anatomy under projects (branch, leaf).
...
## Project Execution Anatomy (Branch / Leaf) — Governed

### Purpose

Represent governed execution structure inside a Project (Tree/Sapling), enabling deterministic "module → actions" planning without diluting Project lifecycle semantics.

### Definitions (binding)

- **Branch:** A structural module under a Project. Branches organize work into coherent units (e.g., "Module 1: Positioning").
- **Leaf:** An executable action item under a Branch. Leaves represent concrete work outputs (e.g., "Draft landing page headline variants").

### Canonical anatomy (non-negotiable)

```
Tree/Sapling (project) → Branch → Leaf
```

### Parent / child rules (binding)

- `branch.parent_artifact_id` MUST reference a project artifact
- `leaf.parent_artifact_id` MUST reference a branch artifact
- Branch MUST NOT parent: forest, thicket, flower, snapshot, restart, journal, leaf, or another branch
- Leaf MUST NOT parent any artifact type

### Flower exclusion (binding)

- Flowers are not part of Project execution trees
- Flowers must never appear under Projects, Branches, or Leaves
- Flowers remain lightweight, non-execution artifacts outside the Branch/Leaf anatomy

### Lifecycle and history interaction

- Branch and Leaf do not introduce new lifecycle stages
- Snapshot semantics remain lifecycle-only for projects
- Branch/Leaf must not dilute Snapshot meaning
...
Instruction packs are structured instruction extensions for GPT front-ends. They allow rich, detailed behavioral rules to be stored and loaded dynamically, extending base system instructions beyond character limits.

- **Extension table:** `qxb_artifact_instruction_pack`
- **Purpose:** Store external instruction content (invariants, rules, templates, examples) that GPT front-ends load at session initialization
- **Scope constraint:** Each instruction pack has a `scope` (e.g., `global`, `view:list`, `view:detail`, `action:save`)
- **Uniqueness:** One active instruction_pack per (workspace_id, scope) enforced by partial unique index
- **Parent rules:** `parent_artifact_id` MUST be NULL (instruction_packs are root-level, not parented to other artifacts)
- **Mutability:** Mutable — content can be updated, packs can be deactivated or replaced
- **Content structure (content.scope required):**
  - `pack_version` — Version identifier for the instruction pack
  - `scope` — Scope key (must match extension table scope)
...
- **Tags:** Should include `scope:<scope_value>` for filtering
- **Initialization protocol:** GPT front-ends call `artifact.list` with `artifact_type=instruction_pack` at session start, filter by scope, and merge into session memory
...
| `artifact_type` | text | project \| snapshot \| restart \| journal \| instruction_pack |
...
| `11_GLOSSARY.md` | Definitions of terms (artifact, snapshot, restart, lifecycle vs state, etc.); includes Branch/Leaf definitions, lineage rules, and Flower exclusion note |
...
- Define `Qxb_Artifact_Instruction_Pack` fields: scope, pack_version, active flag
...
- Define canonical RLS rules for each table (artifact, project, snapshot, restart, journal, instruction_pack)
...
1. Finalize Kernel v1 schemas for `Qxb_Artifact_Project`, `_Snapshot`, `_Restart`, `_Journal`, `_Instruction_Pack`
...
**End of North Star Document v0.3**
