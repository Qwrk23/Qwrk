...
Additional first-class artifact types may exist in the system and database but are not required for Kernel v1 MVP execution until explicitly scheduled. These include structural containers (e.g., forest, thicket) and execution anatomy under projects (branch, limb, leaf).
...
## Project Execution Anatomy (Branch / Limb / Leaf) — Governed

### Purpose

Represent governed execution structure inside a Project (Tree/Sapling), enabling deterministic "module → workstream → actions" planning without diluting Project lifecycle semantics.

### Definitions (binding)

| Artifact | Definition | Example |
|----------|------------|---------|
| **Branch** | Strategic or functional module under a Project | "Backend Development" |
| **Limb** | Coherent workstream or phase within a Branch | "Authentication System" |
| **Leaf** | Executable action item under a Branch or Limb | "Implement JWT token refresh" |
...
- Flowers must never appear under Projects, Branches, Limbs, or Leaves
- Limbs must never appear under Flowers
- Flowers remain lightweight, non-execution artifacts outside the Branch/Limb/Leaf anatomy

### Lifecycle and history interaction

- Branch, Limb, and Leaf do not introduce new lifecycle stages
- Snapshot semantics remain lifecycle-only for projects
- Branch/Limb/Leaf must not dilute Snapshot meaning
...
| `11_GLOSSARY.md` | Definitions of terms (artifact, snapshot, restart, lifecycle vs state, etc.); includes Branch/Limb/Leaf definitions, lineage rules, and Flower exclusion note |
...
**End of North Star Document v0.4**
