# Schema Reference — Kernel v1 (v2.9)

**Source**: `docs/schema/LIVE_DDL__Kernel_v1__2026-01-04.sql` (DDL v2.9)
**Date**: 2026-03-07
**Version**: v2.9
...
**19 tables + 1 VIEW total.** All tables have **RLS enabled** (deny-by-default with explicit policies).
...
### Dependency Table
15. [qxb_artifact_dependency](#qxb_artifact_dependency) — Leaf-to-Leaf Dependency Tracking

### Semantic Type System (T69)
16. [qxb_semantic_type_registry](#qxb_semantic_type_registry) — Semantic Classification Vocabulary
17. [qxb_semantic_type_audit](#qxb_semantic_type_audit) — Semantic Type Change Audit Log

### System Tables
18. [qxb_artifact_type_registry](#qxb_artifact_type_registry) — Type Registry
19. [qxb_artifact_type_registry_audit](#qxb_artifact_type_registry_audit) — Type Registry Audit Log
20. [qxb_gateway_acl](#qxb_gateway_acl) — Gateway Access Control
...
| `artifact_type` | text | NOT NULL | — | Type discriminator (CHECK v7: 14 types) |
...
| `semantic_type_id` | uuid | conditional | — | FK to `qxb_semantic_type_registry`. Conditional NOT NULL: REQUIRED for top-level types (project, snapshot, journal, restart); NULL for all others. |
...
**Check Constraints**:
- **artifact_type (CHECK v7)**: IN (`'project'`, `'journal'`, `'restart'`, `'snapshot'`, `'grass'`, `'thorn'`, `'forest'`, `'thicket'`, `'flower'`, `'branch'`, `'leaf'`, `'instruction_pack'`, `'limb'`, `'twig'`) — 14 types. Note: `'video'` is NOT in CHECK despite `qxb_artifact_video` table existing.
- **priority**: `>= 1 AND <= 5`
...
- **semantic_type_required_for_top_level (conditional)**: `artifact_type NOT IN ('project', 'snapshot', 'journal', 'restart') OR semantic_type_id IS NOT NULL` — top-level types MUST have a semantic_type_id; non-top-level types may be NULL.
...
| Type | Extension Table | CHECK v7 | Mutability | semantic_type_id | Purpose |
|------|----------------|----------|------------|------------------|---------|
...
| `video` | `qxb_artifact_video` | **No** | UPDATE allowed | NULL allowed | Long-form media (NOT in CHECK v7) |

### Gateway Type Registry Boundary (2026-02-20)

The following types exist in CHECK v7 but are **intentionally blocked** at the Gateway layer pending activation:
...
4. **`qxb_artifact.artifact_type` CHECK**: Was 10 types (included `video`). Now CHECK v7: 14 types (`video` NOT in CHECK; `branch`, `leaf`, `instruction_pack`, `limb`, `twig` added).
