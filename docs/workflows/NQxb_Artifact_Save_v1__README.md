...
**v1.2 LOCKED** - Production-ready with error short-circuiting, deterministic ID propagation, lifecycle alignment, and extension UPSERT.

## v1.2 Locked Improvements

### 1. Error Short-Circuiting (Hard Stop on Errors)
- Added `Guard_Error_ShortCircuit` IF node after `Validate_Request`
- Any error envelope (ok=false) immediately routes to final output
- NO DB operations execute if validation or immutability fails
- Prevents error envelopes from falling through to DB nodes

### 2. Deterministic Artifact ID Propagation (INSERT)
- Added `Normalize_Saved_ID` Code node after `DB_Insert_Spine`
- Extracts artifact_id with fallbacks: `artifact_id || id || data[0].artifact_id || data[0].id`
- All extension INSERT nodes now use `{{$json.saved_artifact_id}}`
- Query call uses `saved_artifact_id` for deterministic retrieval
- Eliminates fragile assumptions about Supabase response shape

### 3. Lifecycle Alignment (project artifacts)
- Modified `Normalize_Request` to align `lifecycle_status` (spine) with `lifecycle_stage` (extension)
- **Rule**: For project INSERT, if `lifecycle_stage` provided but `lifecycle_status` missing, copy `lifecycle_stage` → `lifecycle_status`
- **Authoritative field**: `lifecycle_status` on qxb_artifact spine
- **Extension field**: `lifecycle_stage` on qxb_artifact_project mirrors spine status
- Prevents drift between spine and extension lifecycle fields
- Valid values: seed, sapling, tree, retired

### 4. Extension UPSERT on UPDATE (Missing Row Handling)
- Added `Check_Project_Extension_Exists` and `Check_Journal_Extension_Exists` IF nodes
- **IF extension row found**: Continue to Merge_PATCH → DB_Update (existing behavior)
- **IF NOT found**: Route to `Prepare_Insert_Missing_[Type]_Extension` → `DB_Insert_Missing_[Type]_Extension`
- Auto-creates missing extension rows with provided values + defaults
- **Default values**:
  - Project: lifecycle_stage=seed, operational_state=active
  - Journal: entry_text=null, payload=null
- Maintains data integrity even if extension row was deleted or never created
...
```
1. In → 2. Normalize_Request (with lifecycle alignment)
   ↓
3. Validate_Request
   ↓
4. Guard_Error_ShortCircuit (IF node)
   ↓ (ok=false)                  ↓ (ok=true)
[Direct to output]         Switch_InsertOrUpdate
                              ↓UPDATE      ↓INSERT
                         Check_Immutability  DB_Insert_Spine
                              ↓                  ↓
                         [Error exits]     Normalize_Saved_ID
                              ↓                  ↓
                         Fetch_Existing    Switch_Type_For_Insert
                              ↓                  ↓
                         [NOT_FOUND exits] Extension_INSERTs (use saved_artifact_id)
                              ↓
                         Merge_PATCH_Spine
                              ↓
                         DB_Update_Spine
                         ...
                         Fetch_Extension
                              ↓
                         Check_Extension_Exists (IF)
                           ↓found          ↓missing
                      Merge_PATCH      Prepare_Insert_Missing
                           ↓                   ↓
                      DB_UPDATE          DB_INSERT_Missing
                                  ↓
                         Prepare_Query_Call
                              ↓
...
## Locked Status & Regression Testing

### Locked Status

This workflow is **LOCKED** as of v1.2 and considered production-ready. All known edge cases have been addressed:

✅ Validation errors short-circuit (no DB writes)
✅ Immutability errors short-circuit (no DB writes)
✅ NOT_FOUND errors short-circuit (no DB writes)
✅ INSERT artifact_id propagation is deterministic
✅ Project lifecycle fields stay aligned
✅ Missing extension rows are auto-created on UPDATE

### Required Regression Tests Before Unlocking

Before making any changes to this workflow, run these regression tests:

1. **Validation Error Short-Circuit**: Submit invalid request → verify NO DB writes occurred
2. **Immutability Short-Circuit**: Attempt UPDATE on snapshot → verify NO DB writes occurred
3. **Deterministic INSERT ID**: Create project → verify extension uses correct artifact_id
4. **Lifecycle Alignment**: Create project with only lifecycle_stage → verify spine.lifecycle_status matches
5. **Missing Extension UPSERT**: UPDATE project artifact with deleted extension row → verify row is recreated
6. **PATCH Semantics**: UPDATE with partial fields → verify unspecified fields preserved
...
- **v1.2** (2026-01-01): Locked Production-Ready Improvements
  - ✅ **Error short-circuiting** (Guard_Error_ShortCircuit IF node prevents fallthrough)
  - ✅ **Deterministic artifact_id propagation** (Normalize_Saved_ID with fallbacks)
  - ✅ **Lifecycle alignment** (project lifecycle_status ↔ lifecycle_stage sync)
  - ✅ **Extension UPSERT** (auto-creates missing extension rows on UPDATE)
  - 🔒 **LOCKED STATUS** - Workflow is production-ready and regression-tested
...
| lifecycle_status | qxb_artifact | text | Optional | Optional (PATCH)<br>*For project artifacts, auto-aligned with extension.lifecycle_stage on INSERT* |
