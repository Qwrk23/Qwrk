...
QSB.ui = {
  ...
  /** @type {string|null} Timestamp key of last auto-pasted execution (double-paste guard) */
  ...
  render: function () {
    ...
    // --- Status + Controls Bar ---
    var currentProfile = QSB.getSelectedProfile();
    var userName = currentProfile.display_name || currentProfile.label;

    var indicatorClass = "idle";
    var statusText = userName + " \u2014 No staged operation";
    ...
    if (s.isExecuting) {
      ...
      statusText = userName + " \u2014 1 staged: " + (s.stagedPayload.gw_action || "unknown");
      statusActive = true;
    }
  ...
  _bindEvents: function () {
    ...
    // Workspace selection + auto-paste toggle
  ...
  /**
   * Insert pre-formatted text into ChatGPT's input (no fencing applied).
   * Used by autoPasteResponse to inject identity prefix + fenced JSON.
   */
  _insertIntoChatRaw: function (text, triggerBtn) {
    if (!text) return;
  ...
  /**
   * Auto-paste Gateway response into ChatGPT input if setting is enabled.
   * Called from executor.js after each terminal Gateway response.
   *
   * Safety rules:
   *   - Only runs if QSB.autoPaste === true
   *   - Double-paste guarded by _lastAutoPastedKey
   *   - If chat input has existing text, skips (does not overwrite)
   *   - Does NOT auto-send — user must press Enter
   */
  autoPasteResponse: function (responseData) {
    if (!QSB.autoPaste) return;
    if (!responseData) return;

    // Double-paste guard: use latest log entry timestamp
    ...
    var input = document.querySelector("#prompt-textarea")
      || document.querySelector('[contenteditable="true"]');
    if (!input) {
      console.log("QSB auto-paste: no chat input found");
      return;
    }

    // Safety: skip if input already has content
    var existingText = (input.textContent || "").trim();
    if (existingText.length > 0) {
      console.log("QSB auto-paste: skipped (chat input not empty). Use manual Insert button.");
      return;
    }

    // Build identity-prefixed response for Q context
    var profile = QSB.getSelectedProfile();
    var displayName = profile.display_name || profile.label || "unknown";
    var action = (responseData.gw_action || responseData.data && responseData.data.gw_action || "execute");
    var artifactId = this._extractArtifact(responseData);
    var shortId = (artifactId && artifactId.artifact_id) ? artifactId.artifact_id.substring(0, 8) : "";
    var identityLine = "[QSB | " + displayName + " | " + action + (shortId ? " \u2192 " + shortId : "") + "]\n";

    var jsonText = JSON.stringify(responseData, null, 2);
    this._insertIntoChatRaw(identityLine + "```json\n" + jsonText + "\n```\n", null);
    console.log("QSB auto-paste: response inserted into chat input (identity: " + displayName + ")");

    // Auto-dismiss: clear response window since payload was delivered to chat
    QSB.state.executionLog = [];
    this._logExpanded = {};
