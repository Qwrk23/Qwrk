...
| T176 | **Beta Active Launch Program (Qwrk Lite)** | **Critical** | 126b | MANUS REVIEWED + REFINEMENTS LOCKED | **MASTER PROGRAM.** Sapling: `4cac82b5` (v3). Restart snapshot: `407df01b`. Refinement snapshot: `a68c4429` (Manus + Q validated). Execution protocol + gateway parity rule + beta gates locked on sapling content. 6 branches: **A** Infra (`ef366663`, plan `08d5fb37`, **A8 Telemetry = beta gate**), **B** Provisioning (`d23c20cf`, plan `126bde08`, **15/15 leaves created**), **C** Onboarding (`2eee0770`, plan `19b16098`), **D** Acquisition (`b71852ef`, plan `98f2bdb2`), **E** Validation (`16c90112`, plan `33d58b8c`), **F** UX System (`26d794f7`, plan `1dfd3118`). Manus refinements folded in: identity handshake (C2/C3), workspace indicator (B11), atomic bundle (B15), mental model (C2/C6), telemetry (A8 NEW), recovery path (A3/C8), feedback command (C1). Beta gates: A1 + A8. **NEXT:** Branch A (A1 Gateway parity) is the hard blocker. D1-D2 and F1-F2 can parallel. Weekly milestones defined through post-vacation launch. |
| T177 | Payload Builder IP — Test & Promote to Q Heads | High | 126c | IP UPLOADED — Q DEEP TESTED | **T175 Phase A execution.** IP v1 created (`Qwrk_Inbox/Instruction_Pack__Payload_Builder__v1.md`), uploaded to Q project files, deep testing performed by Joel with Q (results in separate thread). **Lifecycle validated (126b):** journal, project, twig, snapshot. Builder = assembler-only. Known: semantic_type_id silent drop bug. Restart: `cdc621b8`. **NEXT:** Review Q test results → decide if IP needs tweaks → promote to Qwrk Prime SI → Akara/Q@W propagation (T159). |
