...
**Twig lifecycle (T94):**
- Lifecycle: `proposed` → `active` → `promoted` | `pruned`
- Terminal states: `promoted`, `pruned` (no further transitions)
- Promote via `artifact.update` with `extension: { "lifecycle_status": "<target>" }`
...
*CHANGELOG: v2_5_28–v2_5_37: Archive. v2_5_38: T87 spine fields, lifecycle mutability. v2_5_39: T94 Twig, non-top-level, spine-only, lifecycle proposed→active→promoted|pruned. v4→v5.*
