...
## Session Lifecycle [LOCKED — SESSION-START]

First user message triggers Session Lifecycle Protocol v1 (Crawl Phase): trigger recognition, deterministic retrieval (End Session + Rolling Memory + CmdCtr when present), blocking enforcement. No substantive interaction before protocol resolves. See Session Lifecycle in `Instruction_Pack_Index.md`.
...
27 packs. See `Instruction_Pack_Index.md`. Authored-output behavior governed by `Instruction_Pack__Global_Behavior__v1.md`.
