/**
 * QSB Beta — PrimeExecutionObject Parser
 *
 * Based on Prime QSB v5 parser.
 * Key difference: only requires gw_action (no gw_workspace_id).
 * Workspace ID is injected by the executor from the profile config.
 */

QSB.parser = {

  /** @type {MutationObserver|null} */
  _observer: null,

  /** @type {number|null} debounce timer */
  _debounceTimer: null,

  /** @type {string|null} fingerprint of last staged payload */
  _lastStagedFingerprint: null,

  // --- Lifecycle ----------------------------------------------------------

  start: function () {
    var self = this;

    this._observer = new MutationObserver(function () {
      clearTimeout(self._debounceTimer);
      self._debounceTimer = setTimeout(function () { self.scan(); }, 300);
    });

    var target = document.querySelector("main") || document.body;
    this._observer.observe(target, { childList: true, subtree: true, characterData: true });
  },

  stop: function () {
    if (this._observer) {
      this._observer.disconnect();
      this._observer = null;
    }
    clearTimeout(this._debounceTimer);
  },

  suspend: function () {
    if (this._observer) {
      this._observer.disconnect();
    }
    clearTimeout(this._debounceTimer);
  },

  resume: function () {
    if (this._observer) {
      var target = document.querySelector("main") || document.body;
      this._observer.observe(target, { childList: true, subtree: true, characterData: true });
    }
  },

  resetFingerprint: function () {
    this._lastStagedFingerprint = null;
  },

  // --- Quote Normalization -------------------------------------------------

  _normalizeQuotes: function (text) {
    return text
      .replace(/[\u201C\u201D\u201E\u201F\u2033\u2036]/g, '"')
      .replace(/[\u2018\u2019\u201A\u201B\u2032\u2035]/g, "'");
  },

  // --- Scanning -----------------------------------------------------------

  scan: function () {
    var messages = document.querySelectorAll('[data-message-author-role="assistant"]');
    if (!messages.length) return;

    var last = messages[messages.length - 1];
    var text = this._normalizeQuotes(last.textContent);

    var markerPos = text.indexOf("prime-exec");
    if (markerPos === -1) return;

    // Strategy A: DOM-based — find JSON inside <pre><code>
    var candidate = this._extractFromCodeBlock(last);

    // Strategy B: balanced brace fallback
    if (candidate === null) {
      var afterMarker = text.substring(markerPos);
      candidate = this._extractBalanced(afterMarker);
    }

    if (candidate === null) return;

    // Fingerprint check
    if (candidate === this._lastStagedFingerprint) return;

    this._tryStage(candidate);
  },

  // --- Extraction ---------------------------------------------------------

  _extractFromCodeBlock: function (messageEl) {
    var codeEls = messageEl.querySelectorAll("pre code");
    if (!codeEls.length) return null;

    for (var i = codeEls.length - 1; i >= 0; i--) {
      var text = this._normalizeQuotes(codeEls[i].textContent).trim();
      if (text.charAt(0) === "{" && text.charAt(text.length - 1) === "}") {
        return text;
      }
    }
    return null;
  },

  _extractBalanced: function (text) {
    var start = text.indexOf("{");
    if (start === -1) return null;

    var depth = 0;
    var inString = false;
    var escaped = false;

    for (var i = start; i < text.length; i++) {
      var ch = text.charAt(i);

      if (escaped) {
        escaped = false;
        continue;
      }

      if (ch === "\\") {
        if (inString) escaped = true;
        continue;
      }

      if (ch === '"') {
        inString = !inString;
        continue;
      }

      if (inString) continue;

      if (ch === "{") {
        depth++;
      } else if (ch === "}") {
        depth--;
        if (depth === 0) {
          return text.substring(start, i + 1);
        }
      }
    }

    return null;
  },

  // --- Validation ---------------------------------------------------------

  /**
   * Parse and validate. Beta requires only gw_action (no gw_workspace_id).
   * Workspace ID is injected by the executor from the profile.
   */
  _tryStage: function (candidate) {
    try {
      var parsed = JSON.parse(candidate);

      if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
        return;
      }

      if (parsed.gw_action) {
        this._lastStagedFingerprint = candidate;
        QSB.state.stage(parsed);
      }
    } catch (e) {
      // Invalid JSON — silently skip
    }
  }
};
