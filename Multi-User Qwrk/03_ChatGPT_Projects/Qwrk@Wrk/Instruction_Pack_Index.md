...
| `Instruction_Pack__Session_Lifecycle__QW__v3.md` | /wake + Workbench + Subsession + End Session protocol (DB-backed memory; T185-aware) | Session start (`/wake`), subsession, session end, workbench operations |
...
*Updated: v7 (2026-05-29) — Session Lifecycle pack bumped v2 → v3. Major v3 expansion: added Workbench section (canonical tag-based working-set; add/remove gestures via structured `artifact.update` tags; /wake listing for project/snapshot/twig — journal excluded; T185-aware non-blocking empty handling distinct from first-wake handler), Subsession Protocol (in-conversation clean lane preserving /wake context; 9 trigger phrases with leading/bare-phrase rule; mid-conversation use; 4 lane-close triggers; fresh-tab refusal; no Gateway calls and no persistence by default), and Concept Distinction table (/wake vs Subsession vs Conversation Restart). Trigger column expanded to include subsession and workbench operations. SI head (`qwrk_work_system_instructions_v_2_11.md`) intentionally untouched — flagged as optional follow-on (use `/update-si` if pursued). Source: Q@W TQR locked decisions + 3 wording amendments, 2026-05-29 (tab 4). Previous: `Archive/Instruction_Pack_Index__v6__2026-05-29.md`. v6 (2026-05-12) — Artifact Discovery Playbook v1.2 → v1.3. T209 Crawl-1 landing (Pass 1, Prime + Q@W). Added sections H (Intent Recognition), I (Candidate Scoring v1), J (Ghost-like Demotion), K (Candidate Presentation and Disambiguation), L (Test Corpus Reference). Front-matter `pack_version` corrected from drifted `v1` to `v1.3`. Trigger column expanded to surface fuzzy artifact discovery requests. Manus TQR `Ready with amendments` applied. Source: Artifact Discovery Layer seed `542cf4c1-c6df-4504-8a1e-9ca799a9c38c`, Crawl-0 Audit Snapshot `b532f87c-5b25-4c92-bb41-1a3cfd06022e`. Previous: `Archive/Instruction_Pack_Index__v5__2026-05-12.md`. v5 (2026-05-06) — Session Lifecycle pack bumped v1 → v2 (first-wake T185 mitigation reframed as fallback; aligned with Workspace Bootstrap Bookmark doctrine). v4 (2026-05-05): Added Session Lifecycle pack (Q@W DB-backed memory migration). v3 (2026-03-30): Messaging pack reference updated to ACTIVE alias; Index synchronization governance rule. v2: Trigger column. v1: extraction.*
