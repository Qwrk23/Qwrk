...
**Artifact types:** `project`, `journal`, `restart`, `snapshot`, `instruction_pack`, `branch`, `limb`, `leaf`, `twig`, `person`
...
Full QSB contract: `Instruction_Pack__QSB_Payload_Format__v3.md`.
...
5. Mobile: Raw JSON only — no marker, no fences.
...
- person: `extension.full_name`, `extension.preferred_name`, `extension.relationship_type` REQUIRED. See `Instruction_Pack__Person_Artifact_Save_Capability_Boundary__v1.md` for full schema + mapping rules.

**Payload discipline:** `Instruction_Pack__Payload_Discipline__v2.md`

**Payload Lookup Mandate [LOCKED]:** Before emitting ANY Gateway payload, open the governing instruction pack from `Instruction_Pack_Index.md` and verify the action's required shape. Never emit from memory alone. **Exception:** Fast-capture types (journal, twig saves) may use pre-validated patterns per Payload Discipline §Fast-Capture Carveout.

**Semantic type (T69):**
- `semantic_type_id` REQUIRED on top-level saves (project, snapshot, journal, restart, person). FORBIDDEN for non-top-level (branch, leaf, limb, instruction_pack, twig).
...
**Messaging:** See `Instruction_Pack__Messaging__v2.2.md` for send_email + calendar_event.

Full spec: `QUICK_REFERENCE.md`, `Instruction_Pack__QSB_Payload_Format__v3.md`.

---

## Artifact Tagging Governance
...
To share feedback, request a feature, or report a problem, Q can capture it as a Feedback Snapshot. See `Instruction_Pack__Feedback_Snapshot__v1.md`.
